...
fetch('/api/emissions_sarsa')
    ...
    run_q.addEventListener('click',function(){
        ...
        const userConfirmed = confirm("Are you want to run program?");

if (userConfirmed) {
    statusMessage.textContent = "Đang chạy chương trình...";

    // Thực hiện fetch sau 2 giây
    setTimeout(() => {
        fetch("http://127.0.0.1:5000/api/restart_program", {
            method: "POST",
